...
export class CartPage extends Component {
    buildDOM() {
        ...
        cartPage.innerHTML = `
            <h2 class="title___cart_products text-SemiBold">${ getLocalizedTag('cart') }</h2>
            <section class="cart">
                <div class="cart__products_container"></div>
                <div></div>
                <div class="cart__price">
                    <div class="cart__price_info">
                        <p class="cart__price_info-total text-SemiBold">${ getLocalizedTag('total') }</p>
                        <p class="cart__price_info-total-price text-SemiBold"><span class="currency text-SemiBold">
                          ${ getLocalizedCurrency(getTotalPrice()) }
                        </p>
                    </div>
                    <button class="cart__price_design-button text-SemiBold text-SemiBold">${ getLocalizedTag('proceedToOrder') }</button>
                </div>
            </section>
        `;
